...
class Transformer(nn.Module):
    def __init__(
        ...
        if encoder is None:
            encoder_layer = encoder_layer or TransformerEncoderLayer(
        ...
        if decoder is None:
            decoder_layer = decoder_layer or TransformerDecoderLayer(
                d_model, nhead, dim_feedforward, dropout, activation, normalize_before
            )
    ...
    def forward(self, src, mask, query_embed, pos_embed, **kwargs):
        ...
        memory = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed, **kwargs)

        dec_outputs = self.decoder(
            tgt, memory, memory_key_padding_mask=mask, pos=pos_embed, query_pos=query_embed, **kwargs
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        tgt, query_pos, tgt_key_padding_mask = self.pre_process_tgt(
            tgt, query_pos, tgt_key_padding_mask=None, **kwargs
        )
        ...
        decoder_outputs.update({"hs": output})
...
class TransformerDecoderLayer(nn.Module):
    def __init__(
        self, d_model, n_heads=8, dim_feedforward=2048, dropout=0.1, activation="relu", normalize_before=False
    ):
        super().__init__()
    ...
    def forward(
        ...
        **kwargs,
        ...
        return self.decoder_layer_forward(
            tgt=tgt,
            memory=memory,
            tgt_mask=tgt_mask,
            tgt_key_padding_mask=tgt_key_padding_mask,
