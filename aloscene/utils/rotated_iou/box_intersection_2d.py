...
"""
import torch
from aloscene.utils.rotated_iou.cuda_op.cuda_ext import sort_v
...
def sort_indices(vertices: torch.Tensor, mask: torch.Tensor):
    ...
    mean = torch.sum(vertices * mask.float().unsqueeze(-1), dim=2, keepdim=True) / num_valid.unsqueeze(-1).unsqueeze(
        -1
    )
